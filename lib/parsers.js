...
export function fileExists(filename) {
  // Special case: settlement is now a folder of TSV files
  if (filename === 'settlement-march-2026.xlsx') {
    const paymentsDir = path.join(process.cwd(), 'data', 'new_data', 'amazon_payments')
    return fs.existsSync(paymentsDir) &&
      fs.readdirSync(paymentsDir).some(f => f.endsWith('.txt'))
  }
  const actual = FILE_MAP[filename] || filename
...
export function parseAllSettlement() {
  ...
  // Check which months already have processed JSON
  const existingJsons = fs.existsSync(PROCESSED_DIR)
    ? fs.readdirSync(PROCESSED_DIR).filter(f => f.startsWith('settlement_') && f.endsWith('.json'))
    : []
