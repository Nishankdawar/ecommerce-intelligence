...
export async function runSettlementEngine(month = null) {
  ...
  // 2. Resolve available months — from TXT files (local) or result JSONs (production)
  const { availableMonths: txtMonths, monthRows } = parseAllSettlement()

  // Months available from pre-computed result JSONs on disk
  const jsonMonths = fs.existsSync(PROCESSED_DIR)
    ? fs.readdirSync(PROCESSED_DIR)
        .filter(f => f.match(/^settlement_result_\d{4}-\d{2}\.json$/))
        .map(f => f.replace('settlement_result_', '').replace('.json', ''))
        .sort()
    : []

  const availableMonths = txtMonths.length > 0 ? txtMonths : jsonMonths
