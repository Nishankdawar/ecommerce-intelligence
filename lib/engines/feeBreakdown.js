...
export async function runFeeBreakdownEngine(month = null) {
  ...
  const _resolveMonth = (m) => {
    ...
    const files = fs.readdirSync(PROCESSED_DIR)
      .filter(f => f.match(/^feeBreakdown_result_\d{4}-\d{2}\.json$/))
      .sort()
    return files.length > 0 ? files[files.length-1].replace('feeBreakdown_result_', '').replace('.json', '') : null
  }
  const _resolvedMonth = _resolveMonth(month)
  const _rPath = _resolvedMonth ? path.join(PROCESSED_DIR, `feeBreakdown_result_${_resolvedMonth}.json`) : null
