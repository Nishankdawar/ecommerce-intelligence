import { parseAllSettlement, parseCSV, parseNum, parseAllMTRB2C } from '../parsers.js'
...
export async function runProfitabilityEngine(month = null) {
  const _cacheKey = `profitability_${month || 'latest'}`
  ...
  const _resolveMonth = (m) => {
    ...
    const files = fs.readdirSync(PROCESSED_DIR)
      .filter(f => f.match(/^profitability_result_\d{4}-\d{2}\.json$/))
      .sort()
    return files.length > 0 ? files[files.length-1].replace('profitability_result_', '').replace('.json', '') : null
  }
  const _resolvedMonth = _resolveMonth(month)
  const _rPath = _resolvedMonth ? path.join(PROCESSED_DIR, `profitability_result_${_resolvedMonth}.json`) : null
  ...
  const { rows: allSettlement, availableMonths: settlementMonths, monthRows } = parseAllSettlement()
  const selectedMonth = month && settlementMonths.includes(month) ? month : settlementMonths[settlementMonths.length - 1]
  const settlement = selectedMonth ? (monthRows[selectedMonth] || []) : allSettlement
  ...
  const _outPath = path.join(PROCESSED_DIR, `profitability_result_${month || 'latest'}.json`)
